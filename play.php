...
switch ($action) {
	...
	$id = isset($_GET["id"]) ? $_GET["id"] : "0";
		if($_GET["id"] == "0") {
			return Main::redirect(array("danger",e("ID Room salah.")),"/index.php");
		}
		$id = $_GET["id"];
		...
		return Main::redirect(array("danger",e("Masukkan data diri.")),"/play.php?id=".$groom['id']."&action=masuk&tahap=1");
		}
		if(!isset($_SESSION['karakter'])) {
			return Main::redirect(array("danger",e("")),"/play.php?id=".$groom['id']."&action=masuk&tahap=2");
		} 
	...
	if(!isset($_GET["id"])) {
		...
		if($groom["pemain"] == 0) {
			return Main::redirect(array("danger",e("Room sudah penuh.")),"/index.php");
		}
	$tahap = isset($_GET["tahap"]) ? $_GET["tahap"] : "1";
	if($tahap == 1 ){
	if(isset($_POST["submitnama"])) {
		if(empty($_POST["namapemain"])){
			return Main::redirect(array("danger",e("Data belum diisi.")),"/play.php?id=".$groom['id']."&action=masuk&tahap=1");
		}
		$cek_user=mysqli_num_rows(mysqli_query($connection, "SELECT * FROM pemain WHERE nama='$_POST[namapemain]'"));
		if ($cek_user > 0) {
        	return Main::redirect(array("danger",e("Sudah ada yang menggunakan nama tersebut.")),"/play.php?id=".$groom['id']."&action=masuk&tahap=1");
		}
		if(isset($_SESSION["masuk"])){
			return Main::redirect("","/play.php?id=".$groom['id']."&action=masuk&tahap=2");
		}
...
<section>
		...
		<?php echo Main::message() ?>
  		<form role="form" class="live_form form" id="login_form" method="post" action="/play.php?id=<?php echo $groom["id"] ?>&action=masuk&tahap=1">   
	...
	if(isset($_SESSION['karakter'])){
		return Main::redirect(array("danger",e("Anda sudah memiliki karakter.")),"/play.php?id=".$groom['id']."");
		}
	if($groom["pemain"] == 0) {
			return Main::redirect(array("danger",e("Room sudah penuh.")),"/index.php");
		}
	...
	<div class="centered form">
	  ...
	  <a class="btn btn-primary" href="/play.php?id=<?php echo $groom["id"] ?>">Start</a>
	  </form>
	</div>
	</section>
	<?php
	...
	if($uroom){
		return Main::redirect(array("success",e("Berhasil mengaktifkan Hasil vote.")),"/play.php?id=".$groom["id"]."");
	}
	return Main::redirect(array("danger",e("Gagal.")),"/play.php?id=".$groom["id"]."");
	break;
	...
	if($uroom){
		return Main::redirect(array("success",e("Berhasil mengaktifkan Siang.")),"/play.php?id=".$groom["id"]."");
	}
	return Main::redirect(array("danger",e("Gagal.")),"/play.php?id=".$groom["id"]."");
	break;
	...
	if($uroom){
		return Main::redirect(array("success",e("Berhasil mengaktifkan Malam Hari.")),"/play.php?id=".$groom["id"]."");
	}
	return Main::redirect(array("danger",e("Gagal.")),"/play.php?id=".$groom["id"]."");
	break;
	case "adminvote" :
	...
	if($uroom){
		return Main::redirect(array("success",e("Berhasil mengaktifkan vote.")),"/play.php?id=".$groom["id"]."");
	}
	return Main::redirect(array("danger",e("Gagal.")),"/play.php?id=".$groom["id"]."");
	break;
	case "vote2" :
	...
	if($gpemain["hidup"] == 0) {
		return Main::redirect(array("danger",e("Anda sudah mati.")),"/play.php?id=".$groom['id']."");
	}
	if($gpemain["karakter"] != "Seer") {
		return Main::redirect(array("danger",e("Anda bukan Seer.")),"/play.php?id=".$groom["id"]."");
	}
	if($gpemain["terawang"] == "1"){
		return Main::redirect(array("danger",e("Anda sudah menerawang malam ini. Coba lagi malam berikutnya")),"/play.php?id=".$groom["id"]."");
	} 
	if(isset($_POST["btnterawang"])) {
		...
		if($terawang["seer"] == 1 ) {
			return Main::redirect(array("danger",e("Tidak boleh memilih pemain yang sama")),"/play.php?action=terawang&id=".$groom["id"]."");
		}
		...
		if($terawang["karakter"] == "Werewolf" || $terawang["karakter"] == "Lycan" ){
			return Main::redirect(array("success",e("".$terawang['nama']." = Werewolf ( W )")),"/play.php?id=".$groom["id"]."");
		} else {
			return Main::redirect(array("success",e("".$terawang['nama']." = Villager ( V )")),"/play.php?id=".$groom["id"]."");
		}
	...
	<section>
		<div class="centered form">
	  		<?php echo Main::message() ?> 
	  		<form role="form" class="live_form form" id="login_form" method="post" action="/play.php?action=terawang&id=<?php echo $groom["id"]; ?>">
				<div class="form-group">
...
case "lindungi" :
	...
	if($gpemain["karakter"] != "Guard") {
		return Main::redirect(array("danger",e("Anda bukan Guard.")),"/play.php?id=".$groom["id"]."");
	}
	if($gpemain["lindungi"] == "1"){
		return Main::redirect(array("danger",e("Anda sudah melindungi seseorang malam ini. Coba lagi malam berikutnya")),"/play.php?id=".$groom["id"]."");
	} 
	if(isset($_POST["btnlindungi"])) {
		...
		$terawang = mysqli_fetch_array(mysqli_query($connection, "select * from pemain where nama='$pilihan' AND idroom='$id'"));
		if($terawang["guard"] == 1 ) {
			return Main::redirect(array("danger",e("Tidak boleh memilih pemain yang sama")),"/play.php?action=lindungi&id=".$groom["id"]."");
		}
		...
		if($upilihan && $upilihan2)	{
			return Main::redirect(array("success",e("Anda memilih melindungi ".$terawang['nama']."")),"/play.php?id=".$groom["id"]."");
		}	
	}	
	?>
	<section>
		<div class="centered form">
	  		<?php echo Main::message() ?> 
	  		<form role="form" class="live_form form" id="login_form" method="post" action="/play.php?action=lindungi&id=<?php echo $groom["id"]; ?>">
				<div class="form-group">
...
case "bunuh" :
	...
	if($gpemain["karakter"] != "Werewolf") {
		return Main::redirect(array("danger",e("Anda bukan Werewolf.")),"/play.php?id=".$groom["id"]."");
	}
	if($gpemain["bunuh"] == "1"){
		return Main::redirect(array("danger",e("Anda sudah membunuh seseorang malam ini. Coba lagi malam berikutnya")),"/play.php?id=".$groom["id"]."");
	} 
	if(isset($_POST["btnbunuh"])) {
		$pilihan = $_POST["pilihanbunuh"];
		if(!isset($pilihan)){
			return Main::redirect(array("danger",e("Tentukan pilihanmu.")),"/play.php?action=bunuh&id=".$groom["id"]."");
		}
		$nama = $_SESSION["masuk"];
		$terawang = mysqli_fetch_array(mysqli_query($connection, "select * from pemain where nama='$pilihan' AND idroom='$id'"));
		if($terawang["werewolf"] == 1 ) {
			return Main::redirect(array("danger",e("Tidak boleh memilih pemain yang sama")),"/play.php?action=bunuh&id=".$groom["id"]."");
		}
		...
		if($upilihan && $upilihan2)	{
			return Main::redirect(array("success",e("Anda memilih membunuh ".$terawang['nama']."")),"/play.php?id=".$groom["id"]."");
		}	
	}	
	?>
	<section>
		<div class="centered form">
	  		<?php echo Main::message() ?> 
	  		<form role="form" class="live_form form" id="login_form" method="post" action="/play.php?action=bunuh&id=<?php echo $groom["id"]; ?>">
				<div class="form-group">
...
case "vote" :
	...
	if($gpemain["milih"] == "1"){
		return Main::redirect(array("danger",e("Anda sudah mem-vote. ")),"/play.php?id=".$groom["id"]."");
	} 
	if(isset($_POST["btnvote"])) {
		$pilihan = $_POST["pilihanvote"];
		if(!isset($pilihan)){
			return Main::redirect(array("danger",e("Tentukan pilihanmu.")),"/play.php?action=vote&id=".$groom["id"]."");
		}
		$nama = $_SESSION["masuk"];
		...
		if($upilihan && $upilihan2)	{
			return Main::redirect(array("success",e("Anda mem-vote ".$pilihan."")),"/play.php?id=".$groom["id"]."");
		}	
	}
	?>
	<section>
		<div class="centered form">
	  		<?php echo Main::message() ?> 
	  		<form role="form" class="live_form form" id="login_form" method="post" action="/play.php?action=vote&id=<?php echo $groom["id"]; ?>">
				<div class="form-group">
