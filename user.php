...
$connection = mysqli_connect("173.208.189.34", "adityaww1_wp197", "Putuaditya97");
// Seleksi Database
...
if(!isset($_SESSION['login'])){
	return Main::redirect(array("danger",e("Login terlebih dahulu.")),"/login.php");
}
...
$action = isset($_GET["action"]) ? $_GET["action"] : "user";
	...
	if(empty($_POST["namaroom"]) || empty($_POST["jumlahpemain"])) {
		return Main::redirect(array("danger",e("Data belum diisi.")),"/user.php?action=create");
	}
	...
	if($aa != $ab){
		return Main::redirect(array("danger",e("Jumlah pemain dengan jumlah karakter tidak sama.")),"/user.php?action=create");
	}
	$namaroom = $_POST["namaroom"];
	$cek_user=mysqli_num_rows(mysqli_query($connection, "SELECT * FROM room WHERE nama='$namaroom'"));
		if ($cek_user > 0) {
        	return Main::redirect(array("danger",e("Sudah ada yang menggunakan nama tersebut.")),"/user.php?action=create");
		}
	$namaroom = $_POST["namaroom"];
	...
	if($iroom) {
		return Main::redirect(array("success",e("Room berhasil dibuat.")),"/user.php");
	}
	return Main::redirect(array("danger",e("Gagal.")),"/user.php?action=create");
}
?>
		<section> 
		<div class="centered form">
		<?php echo Main::message() ?>
  			<form role="form" class="live_form form" id="login_form" method="post" action="/user.php?action=create">   
...
<?php break;
	...
	if (isset($_POST['submit'])) {
	if(empty($_POST["namaroom"]) || empty($_POST["jumlahpemain"])) {
		return Main::redirect(array("danger",e("Data belum diisi.")),"/user.php?action=editroom&id=".$groom['id']."");
	}
	$aa = $_POST["jumlahwerewolf"] + $_POST["jumlahseer"] + $_POST["jumlahguard"] + $_POST["jumlahlycan"] + $_POST["jumlahsorcerer"] + $_POST["jumlahvillager"];
	$ab = $jumlahpemain = $_POST["jumlahpemain"];
	if($aa != $ab){
		return Main::redirect(array("danger",e("Jumlah pemain dengan jumlah karakter tidak sama.")),"/user.php?action=editroom&id=".$groom['id']."");
	}
	$namaroom = $_POST["namaroom"];
	$cek_user=mysqli_num_rows(mysqli_query($connection, "SELECT * FROM room WHERE nama='$namaroom' AND id!='$id'"));
		if ($cek_user > 0) {
        	return Main::redirect(array("danger",e("Sudah ada yang menggunakan nama tersebut.")),"/user.php?action=editroom&id=".$groom['id']."");
		}
	$namaroom = $_POST["namaroom"];
	$jumlahpemain = $_POST["jumlahpemain"];
	$jumlahwerewolf = $_POST["jumlahwerewolf"];
	...
	if($iroom) {
		return Main::redirect(array("success",e("Room berhasil diedit.")),"/user.php");
	}
	return Main::redirect(array("danger",e("Gagal.")),"/user.php?action=editroom&id=".$groom['id']."");
}
?>
		<section> 
		<div class="centered form">
		<?php echo Main::message() ?>
  			<form role="form" class="live_form form" id="login_form" method="post" action="/user.php?action=editroom&id=<?php echo $id; ?>">   
        <div class="form-group">
          <label for="email">Nama room*</label>
...
<?php break;
	...
	<div class="container">
	   <div style="text-align:right" class="call-to-action">
	   	  <a href="/user.php?action=resetpemain" class="btn btn-primary btn-lg">Reset pemain</a>
      	  <a href="/user.php?action=create" class="btn btn-primary btn-lg">Create room</a>
		</div>
	</div>
<section>
<div class="main-form">
			...
			<table>
				...
				$rows = mysqli_num_rows($spemain);
					...
					echo "	<tr>
            				...
            				<td><a class='btn btn-primary' href='/play.php?id=".$row['id']."'>Play</a></td>
							<td><a class='btn btn-primary' href='/user.php?action=delete&id=".$row["id"]."'>Delete</a></td>
							<td><a class='btn btn-primary' href='/user.php?action=editroom&id=".$row["id"]."'>Edit</a></td>
							<td><a class='btn btn-primary' href='/user.php?action=resetroom&id=".$row["id"]."'>Reset</a></td>
              			    </tr><tr>
							<td>".$rows." pemain</td>
							</tr>";
					}
  ...
  <?php
	...
	if(!$droom){
		return Main::redirect(array("danger",e("Room gagal dihapus.")),"/user.php");
	}
	return Main::redirect(array("success",e("Room berhasil dihapus.")),"/user.php");
	break;
		case "resetpemain" :
		$resetpemain = mysqli_query($connection, "TRUNCATE TABLE pemain");
		if($resetpemain) {
			return Main::redirect(array("success",e("Pemain telah dihapus dari semua room.")),"/user.php");
		}
		break;
		case "resetroom" :
		if(!isset($_GET["id"])) {
			return Main::redirect(array("danger",e("ID salah.")),"/user.php");
		}
		$id_delete = $_GET['id'];
		...
		if($aaaa) {
			return Main::redirect(array("success",e("Karakter telah direset.")),"/user.php");
		}
		break;
}
?>
